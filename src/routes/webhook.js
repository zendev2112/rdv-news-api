...
router.post('/airtable/publish', async (req, res) => {
  try {
    ...
    if (!recordId) {
      ...
      return res
        .status(400)
        .json({ success: false, error: 'Missing recordId parameter' })
    }
    ...
    if (!sectionId) {
      ...
      return res
        ...
        .json({
          success: false,
          error: 'Missing sectionId or tableName parameter',
        })
    }

    logger.info(
      `Fetching Airtable record ${recordId} from section ${sectionId}`,
    )
...
router.post('/status-change', async (req, res) => {
  try {
    ...
    const success = await handlePublishStatusChange(
      recordId,
      tableName,
      sectionId,
    )
