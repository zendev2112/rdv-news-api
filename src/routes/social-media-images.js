...
// Replace the generateSocialMediaImageFile function with this new implementation

/**
 * Create an image with text using direct rendering in a single process
...
async function generateSocialMediaImageFile(imageUrl, title, dateStr, options = {}) {
  try {
    ...
    // Generate image directly (no separate process)
    const canvas = createCanvas(width, height);
    const ctx = canvas.getContext('2d');
    ...
    ctx.drawImage(
      titleTextImage,
      0, 0, titleTextImage.width, titleTextImage.height,
      0, height - 100, width, titleFontSize * 2
    );
    
    // Composite date onto main image
    ctx.drawImage(
      dateTextImage,
      0, 0, dateTextImage.width, dateTextImage.height,
      0, height - 40, width, dateFontSize * 2
    );
    
    // Save to final image path
...
async function generateImageWithGm(imageUrl, text, dateStr, options = {}) {
  try {
    ...
    try {
      ...
      logger.warn(`GraphicsMagick failed or not installed: ${gmError.message}`);
      logger.info('Using Canvas fallback instead...');
      
      // Canvas fallback approach
      const canvas = createCanvas(width, height);
      const ctx = canvas.getContext('2d');
      
      // Load the source image we already downloaded
      ...
      // Save to output file (same path we planned to use for GM)
      ...
      try {
        ...
        logger.warn(`Could not delete source image: ${cleanError.message}`);
      }
      
      logger.info(`Image generated with Canvas fallback at ${outputPath}`);
      return outputPath;
    }
  } catch (error) {
