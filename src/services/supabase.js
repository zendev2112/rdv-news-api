...
if (decodedKey.role !== 'service_role') {
  console.warn(
    'WARNING: Not using a service_role key. This may cause RLS policy violations.',
  )
}
...
const sectionNameToId = {
  ...
  'Cine y Series': 'cine-series', // ✅ CRITICAL: Verify this exists
  'Historia y Literatura': 'historia-literatura',
}
...
async function publishArticle(airtableRecord) {
  try {
    ...
    logger.debug(
      `Attempting to upsert article with airtable_id: ${airtableRecord.id}`,
    )
    ...
    try {
      ...
      result = await supabase.from('articles').upsert(articleData, {
      ...
      if (!result.data || result.data.length === 0) {
        logger.info(
          'Upsert succeeded but no data returned, fetching article data separately',
        )
        ...
        if (!selectResult.data)
          throw new Error('Could not find the article after upsert')
    ...
    try {
      ...
      logger.info(
        `Creating relationship between article ${articleId} and section ${sectionId}`,
      )
      ...
      if (checkError && !checkError.message.includes('No rows found')) {
        logger.error(
          'Error checking for existing article-section relationship:',
          checkError,
        )
        ...
        const isPrimary =
          !sectionsError && (!existingSections || existingSections.length === 0)
        ...
        if (relationError) {
          logger.error(
            'Error creating article-section relationship:',
            relationError,
          )
        } else {
          logger.info(
            `Created article-section relationship successfully (primary: ${isPrimary})`,
          )
        }
      } else {
        logger.info(
          'Article-section relationship already exists, skipping creation',
        )
      }
    } catch (relationError) {
      logger.error(
        'Error managing article-section relationship:',
        relationError,
      )
